    ...
    >
      <div className="flex justify-between items-center mx-auto px-4 sm:px-6 lg:px-30 h-20 max-w-[1600px]">
        ...
        <div className="md:hidden flex items-center">
          <button
            onClick={toggleMenu}
            className="inline-flex items-center justify-center p-2.5 rounded-xl border border-white/10 bg-white/5 text-gray-300 hover:text-white hover:bg-white/10 focus:outline-none transition-all active:scale-95"
          ...
          >
            ...
            {!isOpen ? (
              ...
              <svg
                className="block h-5 w-5 text-triton-red"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`md:hidden fixed inset-x-0 top-20 bg-neutral-950/95 backdrop-blur-2xl border-b border-white/10 transition-all duration-500 ease-in-out overflow-y-auto ${isOpen ? "h-[calc(100vh-80px)] opacity-100 translate-y-0" : "h-0 opacity-0 -translate-y-10 pointer-events-none"
          }`}
      >
        <div className="px-6 py-10 flex flex-col items-center gap-6 min-h-full">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              onClick={() => setIsOpen(false)}
              className="text-white block px-4 py-2 text-xl sm:text-2xl font-black uppercase tracking-wider hover:text-triton-red transition-all duration-300 active:scale-95"
            >
              {link.name}
            </a>
          ))}
